...
issues-label-trend = Verlauf
issues-trend-tooltip = Ereignisaufkommen im gewählten Zeitraum
...
issues-list-filter-environment = Nach Umgebung filtern
issues-list-environment-all = Alle Umgebungen
...
issue-detail-in-app-only = Nur App-Frames
issue-detail-reverse-order = Reihenfolge umkehren
issue-detail-copy = Kopieren
issue-detail-copy-frame = Diesen Frame kopieren
issue-detail-library-frames = { $count ->
    [one] { $count } Bibliotheks-Frame
   *[other] { $count } Bibliotheks-Frames
}
...
issue-detail-view-replay = Replay anzeigen
issue-detail-view-trace = Trace anzeigen
...
issue-detail-external-tracker = Externer Tracker
issue-detail-view-on = Ansehen auf
flash-tracker-create-failed = Der Tracker-Issue konnte nicht angelegt werden. Prüfe Token und Repository der Integration und versuche es erneut.
flash-tracker-config-incomplete = Dieser Tracker-Integration fehlt ein Repository oder ein Token. Korrigiere das in den Integrationseinstellungen.
