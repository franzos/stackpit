...
-- raw handle. A read of the database yields hashes, not cookie values an
-- attacker could replay against this server. Lookups derive the hash on
-- the fly from the cookie's raw 32 bytes.
--
-- Why encrypted at rest: this table holds live IdP credentials. A read of
-- the database file shouldn't yield bearer tokens that an attacker can
-- replay against Hydra. Tokens are AES-256-GCM ciphertext (see
-- crate::crypto) with the raw (not hashed) handle mixed into AAD --
-- blob-swapping between rows fails decryption, and forging a hash
-- preimage doesn't yield plaintext without the master key. `key_id`
-- exists for future key rotation; today's key is `0`.
--
