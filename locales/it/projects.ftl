...
projects-repos-forge-override = Tipo di forge
projects-repos-forge-detected = Rilevato automaticamente
projects-repos-forge-override-help = Da impostare solo se il tipo rilevato è sbagliato, di norma su un'istanza self-hosted il cui hostname non dice nulla sul forge che esegue.
projects-repos-prefix = Prefisso del percorso
projects-repos-prefix-placeholder = services/api/
projects-repos-prefix-help = Quali frame dello stack appartengono a questo repository, confrontati con l'inizio del nome file di un frame. Lascia vuoto in un progetto con un solo repository. Non appena un repository qui ha un prefisso, vale solo la corrispondenza per prefisso e i repository senza prefisso smettono di produrre link al codice.
projects-repos-col-prefix = Prefisso percorso
projects-integrations-reset = Ripristina i valori dell'organizzazione
