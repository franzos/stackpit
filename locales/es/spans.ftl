# Superficie de spans: la lista de spans/traces por proyecto (spans-*) y la
# página de detalle de trace en cascada (trace-detail-*). Reutiliza nav-spans.
# Los strings contados usan plurales tv_count ([one]/[other]).

# --- Sufijo de título ---
spans-title-suffix = — Stackpit

# --- Lista de spans/traces ---
spans-list-empty = No se encontraron spans para este proyecto.
spans-traces-heading = Traces
spans-all-heading = Todos los spans

# --- Tabla de traces ---
spans-col-trace-id = ID de trace
spans-col-root-op = Op raíz
spans-col-root-description = Descripción raíz
spans-col-duration = Duración
spans-col-first-seen = Visto por primera vez
spans-col-last-seen = Visto por última vez

# --- Tabla de spans agregados (agrupados por op/descripción) ---
spans-agg-heading = Operaciones de span
spans-col-count = Recuento
spans-col-p50 = p50
spans-col-p95 = p95
spans-col-avg = Media
spans-agg-truncated = Mostrando las { $count } operaciones de span principales.

# --- Tabla de todos los spans ---
spans-col-span-id = ID de span
spans-col-op = Op
spans-col-description = Descripción
spans-col-timestamp = Marca de tiempo

# --- Paginación (lista de spans) ---
spans-pagination-label = Paginación
spans-traces-pagination-label = Paginación de trazas
spans-traces-count = { $count ->
    [one] { $count } traza
   *[other] { $count } trazas
}
spans-pagination-prev = « Anterior
spans-pagination-next = Siguiente »
spans-count = { $count ->
    [one] { $count } span
   *[other] { $count } spans
}

# --- Lista de traces de toda la organización ---
traces-col-root = Transacción raíz
traces-col-projects = Proyectos
traces-col-transactions = Transacciones
traces-col-errors = Errores
traces-list-empty = No se encontraron traces.
traces-filter-projects-label = ID de proyectos
traces-filter-projects-placeholder = ID de proyectos
traces-filter-period-label = Periodo
traces-filter-multi-label = Abarca más de un proyecto
traces-filter-submit = Filtrar
traces-pagination-label = Paginación de traces
traces-count = { $count ->
    [one] { $count } trace
   *[other] { $count } traces
}
traces-more-projects = { $count ->
    [one] +{ $count } más
   *[other] +{ $count } más
}

# --- Detalle de trace (cascada) ---
# title-prefix/suffix envuelven el trace id dinámico; total/showing-first/of se
# dividen en los límites de { $var } de la línea de metadatos.
trace-detail-title-prefix = Trace
trace-detail-title-suffix = — Stackpit
trace-detail-trace-id-label = trace_id:
trace-detail-total = en total
trace-detail-showing-first = mostrando los primeros
trace-detail-of = de
trace-detail-empty = No se encontraron spans para este trace.
trace-detail-col-span = Span
trace-detail-col-duration = Duración
trace-detail-root-fallback = (raíz del trace)
trace-detail-error-title = error
trace-detail-span-fallback = span
trace-detail-compressed-note = huecos inactivos comprimidos
trace-detail-gap-title = Hueco inactivo colapsado (sin spans activos)
trace-detail-lbl-span-id = ID del span
trace-detail-lbl-parent = Span padre
trace-detail-lbl-status = Estado
trace-detail-lbl-start = Desplazamiento de inicio
trace-detail-correlated-errors = Errores correlacionados
trace-detail-col-level = Nivel
trace-detail-col-title = Título
trace-detail-col-timestamp = Marca de tiempo
trace-detail-span-count = { $count ->
    [one] { $count } span
   *[other] { $count } spans
}
trace-detail-legend-heading = Proyectos
trace-detail-filter-active = filtrado
trace-detail-filter-clear = Mostrar todo
trace-detail-col-project = Proyecto
trace-detail-parent-not-in-view = Span padre fuera de la vista
trace-detail-txn-badge = Transacción
trace-detail-view-events = Todos los eventos de esta traza
